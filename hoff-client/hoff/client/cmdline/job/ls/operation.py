...
class Printer(object):
    def __init__(self, col_names, data, dest=None):
        self.columns = [
            Column(name, data[name], self.format_template) for name in col_names
        ]
    ...
    def __call__(self):
        ...
        for i in range(self.nrows):
            self.printer(
                self.make_line(lambda c: c.fmt_str.format(c.data[i], width=c.width))
            )
        self.print_footer()
...
def run(args):
    columns = args.cols
    PrinterCls = {"table": TablePrinter, "plain": PlainPrinter}[args.format]
