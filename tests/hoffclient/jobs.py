...
class Job(JsonCodable):
    JSON_ATTRS = [
        'project', 'executable', 'id', 'queue', 'template', 'input_set_id', 'num_total_cpus', 'total_physical_memory', 'filter', 'remote_job_id', 'arguments', 'wallclock_limit', 'service_id',
...
class JobClient(SubClient):
    ...
    def list(self, include_deleted=False):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not list visible jobs',
                                response=resp)
    ...
    def get(self, job_id):
        ...
        if resp.status_code != 200:
            raise HoffError('Could get info for job ID "{}"'.format(job_id),
                                response=resp)
        data = resp.json()
    ...
    def add_input(self, job_id, **files):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not upload files for job ID "{}"'.format(job_id),
                                response=resp)
    ...
    def submit(self, job_id):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not submit job ID "{}"'.format(job_id),
                                response=resp)
    ...
    def get_retrieved(self, job_id):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not get retrieved flag for job ID "{}"'.format(job_id),
                                response=resp)
    ...
    def list_output(self, job_id):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not list output files for job ID "{}"'.format(job_id),
                                response=resp)
        return resp.json()
    ...
    def delete(self, job_id):
        ...
        if resp.status_code != 200:
            raise HoffError('Could not delete job ID "{}"'.format(job_id),
                                response=resp)
